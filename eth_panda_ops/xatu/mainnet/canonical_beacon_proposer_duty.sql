create or replace view eth_panda_ops__xatu__mainnet__canonical_beacon_proposer_duty as (
    with 
        (
            select arrayMap(
                x -> toDate(x),  
    ...
    select 
        ...
        *
    from s3(
        'https://data.ethpandaops.io/xatu/' || {network:String} || '/databases/default/canonical_beacon_proposer_duty/' || files_pattern || '.parquet',
        'Parquet',
        $heredoc$
            ...
            proposer_validator_index Nullable(UInt32),
...
comment $comment${
    ...
    "usage": "select * from canonical_beacon_proposer_duty(network = 'mainnet', from='2020-12-01', to='2020-12-03') limit 10",
    "columns": [
        ...
        {"name": "proposer_validator_index", "type": "Nullable(UInt32)"},
