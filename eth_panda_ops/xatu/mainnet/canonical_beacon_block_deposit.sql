create or replace view eth_panda_ops__xatu__mainnet__canonical_beacon_block_deposit as (
    with 
        (
            select arrayMap(
                x -> toDate(x),  
    ...
    select 
        ...
        *
    from s3(
        'https://data.ethpandaops.io/xatu/' || {network:String} || '/databases/default/canonical_beacon_block_deposit/' || files_pattern || '.parquet',
        'Parquet',
        $heredoc$
            ...
            deposit_proof Array(Nullable(String)),
            deposit_data_pubkey Nullable(String),
            deposit_data_withdrawal_credentials Nullable(FixedString(66)),
            deposit_data_amount Nullable(FixedString(16)),
            deposit_data_signature Nullable(String),
...
comment $comment${
    ...
    "columns": [
        ...
        {"name": "deposit_proof", "type": "Array(Nullable(String))"},
        {"name": "deposit_data_pubkey", "type": "Nullable(String)"},
        {"name": "deposit_data_withdrawal_credentials", "type": "Nullable(FixedString(66))"},
        {"name": "deposit_data_amount", "type": "Nullable(FixedString(16))"},
        {"name": "deposit_data_signature", "type": "Nullable(String)"},
