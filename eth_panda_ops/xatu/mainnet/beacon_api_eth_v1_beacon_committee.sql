create or replace view eth_panda_ops__xatu__mainnet__beacon_api_eth_v1_beacon_committee as (
    with 
        (
            select arrayMap(
    ...
    select 
        ...
        *
    from s3(
        'https://data.ethpandaops.io/xatu/' || {network:String} || '/databases/default/beacon_api_eth_v1_beacon_committee/' || files_pattern || '.parquet',
        'Parquet',
        $heredoc$
...
comment $comment${
    ...
    "usage": "select * from beacon_api_eth_v1_beacon_committee(network = 'mainnet', from='2023-09-05', to='2023-09-07') limit 10",
    "columns": [
