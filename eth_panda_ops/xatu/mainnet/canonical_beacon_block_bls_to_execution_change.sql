create or replace view eth_panda_ops__xatu__mainnet__canonical_beacon_block_bls_to_execution_change as (
    with 
        (
            select arrayMap(
                x -> toDate(x),  
    ...
    select 
        ...
        *
    from s3(
        'https://data.ethpandaops.io/xatu/' || {network:String} || '/databases/default/canonical_beacon_block_bls_to_execution_change/' || files_pattern || '.parquet',
        'Parquet',
        $heredoc$
            ...
            exchanging_message_validator_index Nullable(UInt32),
            exchanging_message_from_bls_pubkey Nullable(String),
            exchanging_message_to_execution_address Nullable(FixedString(42)),
            exchanging_signature Nullable(String),
...
comment $comment${
    ...
    "columns": [
        ...
        {"name": "exchanging_message_validator_index", "type": "Nullable(UInt32)"},
        {"name": "exchanging_message_from_bls_pubkey", "type": "Nullable(String)"},
        {"name": "exchanging_message_to_execution_address", "type": "Nullable(FixedString(42))"},
        {"name": "exchanging_signature", "type": "Nullable(String)"},
