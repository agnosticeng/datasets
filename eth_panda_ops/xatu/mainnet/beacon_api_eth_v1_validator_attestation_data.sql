create or replace view eth_panda_ops__xatu__mainnet__beacon_api_eth_v1_validator_attestation_data as (
    with 
        (
            select arrayMap(
    ...
    select 
        ...
        *
    from s3(
        'https://data.ethpandaops.io/xatu/' || {network:String} || '/databases/default/beacon_api_eth_v1_validator_attestation_data/' || files_pattern || '.parquet',
        'Parquet',
        $heredoc$
            ...
            request_date_time Nullable(UInt32),
            request_duration Nullable(UInt32),
            request_slot_start_diff Nullable(UInt32),
...
comment $comment${
    "short": "Contains beacon API validator attestation data from each sentry client attached to a beacon node.",
    "url": "https://ethpandaops.io/data/xatu/schema/beacon_api_/",
    "usage": "select * from beacon_api_eth_v1_validator_attestation_data(network = 'mainnet', from='2023-08-31', to='2023-09-02') limit 10",
    "columns": [
        ...
        {"name": "request_date_time", "type": "Nullable(UInt32)"},
        {"name": "request_duration", "type": "Nullable(UInt32)"},
        {"name": "request_slot_start_diff", "type": "Nullable(UInt32)"},
