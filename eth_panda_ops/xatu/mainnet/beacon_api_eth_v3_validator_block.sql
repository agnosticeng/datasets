create or replace view eth_panda_ops__xatu__mainnet__beacon_api_eth_v3_validator_block as (
    with 
        (
            select arrayMap(
    ...
    select 
        ...
        *
    from s3(
        'https://data.ethpandaops.io/xatu/' || {network:String} || '/databases/default/beacon_api_eth_v3_validator_block/' || files_pattern || '.parquet',
        'Parquet',
        $heredoc$
...
comment $comment${
    ...
    "columns": [
        ...
        {"name": "consensus_payload_value", "type": "Nullable(UInt64)"},
        {"name": "execution_payload_value", "type": "Nullable(UInt64)"},
