create or replace view eth_panda_ops__xatu__mainnet__canonical_beacon_block_voluntary_exit as (
    with 
        (
            select arrayMap(
                x -> toDate(x),  
    ...
    select 
        ...
        *
    from s3(
        'https://data.ethpandaops.io/xatu/' || {network:String} || '/databases/default/canonical_beacon_block_voluntary_exit/' || files_pattern || '.parquet',
        'Parquet',
        $heredoc$
            ...
            voluntary_exit_message_epoch Nullable(UInt32),
            voluntary_exit_message_validator_index Nullable(UInt32),
            voluntary_exit_signature Nullable(String),
...
comment $comment${
    "short": "Contains a voluntary exit from a beacon block.",
    "url": "https://ethpandaops.io/data/xatu/schema/canonical_beacon_/",
    "usage": "select * from canonical_beacon_block_voluntary_exit(network = 'mainnet', from='2020-12-01', to='2020-12-03') limit 10",
    "columns": [
        ...
        {"name": "voluntary_exit_message_epoch", "type": "Nullable(UInt32)"},
        {"name": "voluntary_exit_message_validator_index", "type": "Nullable(UInt32)"},
        {"name": "voluntary_exit_signature", "type": "Nullable(String)"},
