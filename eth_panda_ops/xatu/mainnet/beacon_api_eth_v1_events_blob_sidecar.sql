create or replace view eth_panda_ops__xatu__mainnet__beacon_api_eth_v1_events_blob_sidecar as (
    with 
        (
            select arrayMap(
    ...
    select 
        ...
        *
    from s3(
        'https://data.ethpandaops.io/xatu/' || {network:String} || '/databases/default/beacon_api_eth_v1_events_blob_sidecar/' || files_pattern || '.parquet',
        'Parquet',
        $heredoc$
...
comment $comment${
    "short": "Contains beacon API eventstream “blob_sidecar” data from each sentry client attached to a beacon node.",
    "url": "https://ethpandaops.io/data/xatu/schema/beacon_api_/",
    "usage": "select * from beacon_api_eth_v1_events_blob_sidecar(network = 'mainnet', from='2024-03-13', to='2024-03-15') limit 10",
    "columns": [
