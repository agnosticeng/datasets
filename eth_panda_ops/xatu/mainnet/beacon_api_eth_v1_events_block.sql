create or replace view eth_panda_ops__xatu__mainnet__beacon_api_eth_v1_events_block as (
    with 
        (
            select arrayMap(
    ...
    select 
        ...
        *
    from s3(
        'https://data.ethpandaops.io/xatu/' || {network:String} || '/databases/default/beacon_api_eth_v1_events_block/' || files_pattern || '.parquet',
        'Parquet',
        $heredoc$
...
comment $comment${
    "short": "Contains beacon API eventstream “block” data from each sentry client attached to a beacon node.",
    "url": "https://ethpandaops.io/data/xatu/schema/beacon_api_/",
    "usage": "select * from beacon_api_eth_v1_events_block(network = 'mainnet', from='2023-02-28', to='2023-03-02') limit 10",
    "columns": [
